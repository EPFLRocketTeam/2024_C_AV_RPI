...
int LoRaClass::begin(long frequency, int SPI) {
    ...
    gpioSetPullUpDown(_dio0, PI_PUD_DOWN);

    //configure SS
    gpioSetMode(_ss, PI_OUTPUT);
    gpioWrite(_ss, PI_HIGH);

    //configure & perform reset
    if (_reset != -1) {
        gpioSetMode(_reset, PI_OUTPUT);

        // perform reset
        gpioWrite(_reset, PI_LOW);
        gpioDelay(10000); // wait 10 ms
        gpioWrite(_reset, PI_HIGH);
        gpioDelay(10000);
    }
...
int LoRaClass::packetRssi() {
    ...
    if (_frequency < RF_MID_BAND_THRESHOLD) {
        rssi_offset = RSSI_OFFSET_LF_PORT;
    }else {
        rssi_offset = RSSI_OFFSET_HF_PORT;
    }

    return (readRegister(REG_PKT_RSSI_VALUE) - rssi_offset);
}
...
int LoRaClass::rssi() {
    ...
    return (readRegister(REG_RSSI_VALUE) - rssi_offset);
}
...
void LoRaClass::setSpreadingFactor(int sf) {
    ...
    writeRegister(REG_MODEM_CONFIG_2, (readRegister(REG_MODEM_CONFIG_2) & 0x0f)
            | ((sf << 4) & 0xf0));
    setLdoFlag();
}
...
uint8_t LoRaClass::singleTransfer(uint8_t address, uint8_t value) {
    ...
    gpioWrite(_ss, PI_LOW);
    // DISABLE_INTERRUPTS();
    spiXfer(_spi_handle, buf, buf, 2);
    // ENABLE_INTERRUPTS();
