...
void dump_csv_PropSensors (PropSensors &value, std::ostringstream &stream) {
	...
	dump_csv_float(value.igniter_pressure, stream);
	...
	dump_csv_float(value.igniter_temperature, stream);
	dump_csv_float(value.fuel_inj_temperature, stream);
	dump_csv_float(value.fuel_inj_cooling_temperature, stream);
	dump_csv_float(value.LOX_inj_temperature, stream);
...
void dump_csv_Vector3 (Vector3 &value, std::ostringstream &stream) {
	dump_csv_double(value.x, stream);
	dump_csv_double(value.y, stream);
	dump_csv_double(value.z, stream);
}
void dump_csv_GPSCoord (GPSCoord &value, std::ostringstream &stream) {
...
void dump_csv_NavigationData (NavigationData &value, std::ostringstream &stream) {
	...
	dump_csv_Vector3(value.speed, stream);
...
void dump_csv_Batteries (Batteries &value, std::ostringstream &stream) {
	...
	dump_csv_float(value.hpb_voltage, stream);
	dump_csv_float(value.hpb_current, stream);
}
void dump_csv_CamsRecording (CamsRecording &value, std::ostringstream &stream) {
...
int main (void) {
	...
	std::ifstream stream("../../logs/dump_log_2025-03-09_dry_run.log", std::ios_base::binary);

	std::cout << "av_state.values,av_state.0,av_timestamp,telemetry_cmd.id,telemetry_cmd.value,av_fc_temp,av_amb_temp,stat.adxl_status,stat.adxl_aux_status,stat.bmi_accel_status,stat.bmi_aux_accel_status,stat.bmi_gyro_status,stat.bmi_aux_gyro_status,stat.bmp_status.intr.fifo_wm,stat.bmp_status.intr.fifo_full,stat.bmp_status.intr.drdy,stat.bmp_status.sensor.cmd_rdy,stat.bmp_status.sensor.drdy_press,stat.bmp_status.sensor.drdy_temp,stat.bmp_status.err.fatal,stat.bmp_status.err.cmd,stat.bmp_status.err.conf,stat.bmp_status.pwr_on_rst,stat.bmp_aux_status.intr.fifo_wm,stat.bmp_aux_status.intr.fifo_full,stat.bmp_aux_status.intr.drdy,stat.bmp_aux_status.sensor.cmd_rdy,stat.bmp_aux_status.sensor.drdy_press,stat.bmp_aux_status.sensor.drdy_temp,stat.bmp_aux_status.err.fatal,stat.bmp_aux_status.err.cmd,stat.bmp_aux_status.err.conf,stat.bmp_aux_status.pwr_on_rst,sens.adxl.x,sens.adxl.y,sens.adxl.z,sens.adxl_aux.x,sens.adxl_aux.y,sens.adxl_aux.z,sens.bmi_accel.x,sens.bmi_accel.y,sens.bmi_accel.z,sens.bmi_gyro.x,sens.bmi_gyro.y,sens.bmi_gyro.z,sens.bmi_aux_accel.x,sens.bmi_aux_accel.y,sens.bmi_aux_accel.z,sens.bmi_aux_gyro.x,sens.bmi_aux_gyro.y,sens.bmi_aux_gyro.z,sens.bmp.temperature,sens.bmp.pressure,sens.bmp_aux.temperature,sens.bmp_aux.pressure,prop.N2_pressure,prop.fuel_pressure,prop.LOX_pressure,prop.igniter_pressure,prop.LOX_inj_pressure,prop.fuel_inj_pressure,prop.chamber_pressure,prop.N2_temperature,prop.fuel_temperature,prop.LOX_temperature,prop.igniter_temperature,prop.fuel_inj_temperature,prop.fuel_inj_cooling_temperature,prop.LOX_inj_temperature,prop.chamber_temperature,prop.PRB_state,valves.valve_dpr_pressure_lox,valves.valve_dpr_pressure_fuel,valves.valve_dpr_vent_copv,valves.valve_dpr_vent_lox,valves.valve_dpr_vent_fuel,valves.valve_prb_main_lox,valves.valve_prb_main_fuel,nav.time.year,nav.time.month,nav.time.day,nav.time.hour,nav.time.minute,nav.time.second,nav.time.centisecond,nav.position.lat,nav.position.lng,nav.position.alt,nav.gnss_speed,nav.position_kalman.x,nav.position_kalman.y,nav.position_kalman.z,nav.speed.x,nav.speed.y,nav.speed.z,nav.accel.x,nav.accel.y,nav.accel.z,nav.attitude.x,nav.attitude.y,nav.attitude.z,nav.course,nav.altitude,nav.baro.temperature,nav.baro.pressure,bat.lpb_voltage,bat.lpb_current,bat.hpb_voltage,bat.hpb_current,cams_recording.cam_sep,cams_recording.cam_up,cams_recording.cam_down,event.command_updated,event.calibrated,event.dpr_eth_ready,event.dpr_eth_pressure_ok,event.dpr_lox_ready,event.dpr_lox_pressure_ok,event.prb_ready,event.trb_ready,event.ignited,event.engine_cut_off,event.seperated,event.chute_unreefed,event.ignition_failed,event.catastrophic_failure\n";
